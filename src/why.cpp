...
static QVector<QString>
    obfsucV; /** @brief already obfuscated names, order is meaningful */
QMap<QString, QString> structTypesG; /** useless??? */
static bool obf = true;

static bool eqType(const QDomElement& e1, const QDomElement& e2);

static QString whyApply(const QString& fun, const QVector<QString>& args);
...
static unsigned lookupStruct(const QDomElement& e) {
  ...
  for (i = 0; i < registryStruct.size(); ++i) {
    if (eqType(e, registryStruct.at(i))) return i;
  }
...
static unsigned getStructIdOfTypref(unsigned typref) {
  unsigned id;
  map<unsigned, unsigned>::iterator it = whyStructTyprefToIndex.find(typref);
  if (it == whyStructTyprefToIndex.end()) {
    id = lookupStruct(TypingContext::getType(typref));
    whyStructTyprefToIndex.insert(it, pair<unsigned, unsigned>(typref, id));
    return id;
  } else {
    return it->second;
  }
...
static QString whyName(const QDomElement& ident,
                       const QMap<QString, QString>& enums, bool& var);
...
/**
 * @brief appendPrelude prints Why3 prelude to import all definitions for the B
 * theory.
...
/**
 * @brief saveEnumeration prints type definitions corresponding to enumerated
 * sets and builds translation dictionary from B names to why3 names.
 ...
 *
 */
void saveEnumeration(QTextStream& out, QMap<QString, QString>& enums,
                     QDomElement& set) {
  ...
  if (!enums.contains(setName)) {
    ...
    for (QDomElement elem = enumE.firstChildElement("Id"); !elem.isNull();
         elem = elem.nextSiblingElement("Id")) {
...
void saveWhy3(QDomDocument pog, QFile& why, bool xml, bool obfs) {
  ...
  nbLambda = 0;
  nbSet = 0;
  if (!why.open(QIODevice::WriteOnly)) return;
  QTextStream out(&why);
  ...
  for (elem = root.firstChildElement("Define"); !elem.isNull();
       elem = elem.nextSiblingElement("Define")) {
    for (QDomElement set = elem.firstChildElement("Set"); !set.isNull();
         set = set.nextSiblingElement("Set")) {
      if (isEnumeratedSetElement(set)) saveEnumeration(out, enums, set);
    }
  }
  out << "\n";
  ...
  // Defines declaration
  int count;
  for (count = 1, elem = root.firstChildElement("Define"); !elem.isNull();
  ...
  int goal = 0;
  for (elem = root.firstChildElement("Proof_Obligation"); !elem.isNull();
       elem = elem.nextSiblingElement("Proof_Obligation")) {
    if (elem.firstChildElement("Simple_Goal").isNull()) continue;
    for (QDomElement lh = elem.firstChildElement("Local_Hyp"); !lh.isNull();
  ...
  for (goal = 0, elem = root.firstChildElement("Proof_Obligation");
    ...
    if (xml) {
      ...
      std::cout << "<translate>" << tag.text().toStdString() << "</translate>"
                << std::endl;
    }
    if (!elem.firstChildElement("Simple_Goal").isNull()) {
      ...
      for (QDomElement definition = elem.firstChildElement("Definition");
        ...
        if (!defines.contains(definition.attribute("name"))) {
          throw WhyTranslateException("Unknown definition " +
      ...
      for (QDomElement sg = elem.firstChildElement("Simple_Goal"); !sg.isNull();
        ...
        whySimpleGoal* wsg =
            new whySimpleGoal(sg, simple_goal, goalHyps, sgDefinitions, wh);
...
void saveWhy3(QDomDocument pog, QFile& why,
  ...
  for (elem = root.firstChildElement("Proof_Obligation"); !elem.isNull();
    ...
    if (goals.find(group) == goals.end()) continue;
    QDomElement lh;
    for (lh = elem.firstChildElement("Local_Hyp"); !lh.isNull();
         lh = lh.nextSiblingElement("Local_Hyp")) {
  ...
  for (group = 0, elem = root.firstChildElement("Proof_Obligation");
    ...
    for (j = 0, k = 0, sg = elem.firstChildElement("Simple_Goal");
      ...
      if (j < simple_goals[k]) continue;
      whySimpleGoal* wsg = new whySimpleGoal(
          sg, why3goal, localHyps.values(group), sgDefinitions, wh);
      wsg->collectVarAndTypes(enums);
      wsg->translate(enums);
      wsg->declare(out);
      ++k;
...
void saveWhy3(TypeInfos* typeInfos, QList<QDomElement> hypotheses,
  ...
  for (int i = 0; i < hypotheses.size(); i++) {
    ...
    if (elem.nodeName() == "Set") {
      if (isEnumeratedSetElement(elem)) saveEnumeration(out, enums, elem);
    }
  }
  out << "\n";
...
whyPredicate::whyPredicate(QDomElement _e) { e = _e; }
...
void whyPredicate::collectVarAndTypes(QMap<QString, QString> enums) {
  for (QDomElement e1 = e.firstChildElement(); !e1.isNull();
...
static QString whyName(const QDomElement& ident,
  ...
  if (val == "NAT") return "nat";
  if (val == "NAT1") return "nat1";
  if (val == "INT") return "bounded_int";
  if (val == "STRING") return "string";
  if (val == "MAXINT") {
    return "b_maxint32_value";
  }
  if (val == "MININT") {
    return "b_minint32_value";
  }
  if (enums.contains(val)) return enums.value(val);

  var = true;
  QString name;
  if (val.contains('.'))
    name =
        "renamed_v_" + val.replace('.', '_') + "_" + ident.attribute("suffix");
  else
...
void whyPredicate::collectVarAndTypes(QVector<QString>* variables,
  ...
  if (tag == "Quantified_Pred" || tag == "Quantified_Exp" ||
      tag == "Quantified_Set") {
    ...
    QDomElement vars =
        formula.firstChildElement("Variables").firstChildElement("Id");
    while (!vars.isNull()) {
      bool isVar;
  ...
  if (tag == "Binary_Exp" || tag == "Ternary_Exp") {
    ...
    if (op == "'") {
      collectVarAndTypes(variables, types, formula.firstChildElement(), enums,
                         localHyps);
      return;
    }
    if (op == "<'") {
      ...
      collectVarAndTypes(
          variables, types,
          formula.firstChildElement().nextSiblingElement().nextSiblingElement(),
          enums, localHyps);
      return;
    }
  }
...
QString whyPredicate::translateTypeStruct(const QDomElement& e,
  ...
  if (id == whyStructName.size()) {
    ...
    QDomElement ri;
    bool first;
    for (ri = e.firstChildElement("Record_Item"), first = true; !ri.isNull();
...
QString whyPredicate::translateTypeInfo(QDomElement ti,
                                        const QMap<QString, QString>& enums) {
...
QString whyPredicate::translatePred(const QDomElement formula,
                                    const QMap<QString, QString>& enums) {
...
QString whyPredicate::translateId(const QDomElement& e,
                                  const QMap<QString, QString>& enums) {
  bool var;
...
QString whyPredicate::translateStruct(const QDomElement& formula,
                                      const QMap<QString, QString>& enums) {
  int i_as = nbStruct++;
  QString l;
  ...
  unsigned i_ts;
  {
    ...
    if (!ok) throw WhyTranslateException("Untyped struct expression found.");
    QDomElement e = TypingContext::getType(typref);
    if (e.tagName() != "Unary_Exp") throw WhyTranslateException("Typing error");
  ...
  collectVarAndTypes(&qVariables, &qTypes, formula, enums, qLocalHyps);
  ...
  while (!ri.isNull()) {
    ...
    l += translateTypeInfo(
        TypingContext::getType(child1(ri)).firstChildElement(), enums);
    ri = ri.nextSiblingElement("Record_Item");
  }
  l += ". mem {";
  first = true;
  ...
  while (!ri.isNull()) {
    if (first)
      ...
      l += ";";
    l += "l_" + ri.attribute("label") + "_" + QString::number(i_ts) + " = p_" +
         ri.attribute("label");
    ri = ri.nextSiblingElement("Record_Item");
  }
  l += "} (" + function_name + " ";
  for (int i = 0; i < qVariables.size(); i++) l += qVariables.at(i) + " ";
  l += ") <-> (";
  first = true;
  ri = formula.firstChildElement("Record_Item");
  while (!ri.isNull()) {
...
QString whyPredicate::translateRecord(const QDomElement& formula,
                                      const QMap<QString, QString>& enums) {
  bool ok;
  unsigned typref = formula.attribute("typref", "").toUInt(&ok, 10);
  if (!ok) throw WhyTranslateException("Untyped record found.");
  ...
  for (ri = formula.firstChildElement("Record_Item"), first = true;
       !ri.isNull(); ri = ri.nextSiblingElement("Record_Item")) {
    if (first)
      first = false;
    else
      res += "; ";
...
QString whyPredicate::translateRecordFieldAccess(
    const QDomElement& e, const QMap<QString, QString>& enums) {
  QString res;
  ...
  res = "(" + translate(accessed, enums).at(0) + ".l_" + e.attribute("label") +
        "_" + suffix + ")";

  return res;
}

QStringList whyPredicate::translate(const QDomElement formula,
                                    const QMap<QString, QString>& enums) {
  QStringList res;

  QDomElement fce = formula.firstChildElement();
  QDomElement sce;
  if (!fce.isNull()) sce = fce.nextSiblingElement();

  QDomElement tce;
  if (!sce.isNull()) tce = sce.nextSiblingElement();
  ...
  if (tag == "Binary_Pred" || tag == "Exp_Comparison" || tag == "Binary_Exp" ||
      tag == "Ternary_Exp") {
    if (op == "or")
      ...
      res << "(mem " + translate(fce, enums).at(0) + " (power " +
                 translate(sce, enums).at(0) + "))";
    else if (op == "/<:")
      res << "not(mem " + translate(fce, enums).at(0) + " (power " +
                 translate(sce, enums).at(0) + "))";
    else if (op == "<<:")
      res << "((mem " + translate(fce, enums).at(0) + " (power " +
      ...
      res << "(semicolon " + translate(fce, enums).at(0) + " " +
                 translate(sce, enums).at(0) + ")";
    else if (op == "iterate")
      res << "(iterate " + translate(fce, enums).at(0) + " " +
                 translate(sce, enums).at(0) + ")";
    else if (op == "<+")
      res << "(" + translate(fce, enums).at(0) + " <+ " +
                 translate(sce, enums).at(0) + ")";
    ...
    // Alors prj2 (E, F) est une relation de type P((T*U)*U).
      ...
      res << "{" + translate(fce, enums).at(0) + " with l_" +
                 obfusc(sce.attribute("value")) + " = " +
                 translate(tce, enums).at(0) + "}";
    else
      throw WhyTranslateException(tag + " " + op);
    return res;
  } else if (tag == "Unary_Pred" || (tag == "Unary_Exp")) {
    if (op == "not") {
      res << "not(" + translatePred(fce, enums) + ")";
    } else if (op == "max" || op == "imax") {
      res << "(max " + translate(fce, enums).at(0) + ")";
    } else if (op == "min" || op == "imin") {
      res << "(min " + translate(fce, enums).at(0) + ")";
    } else if (op == "card") {
      res << "(card " + translate(fce, enums).at(0) + ")";
    } else if (op == "dom") {
      res << "(dom " + translate(fce, enums).at(0) + ")";
    } else if (op == "ran") {
      res << "(ran " + translate(fce, enums).at(0) + ")";
    } else if (op == "POW") {
      res << "(power " + translate(fce, enums).at(0) + ")";
    } else if (op == "POW1") {
      res << "(non_empty_power " + translate(fce, enums).at(0) + ")";
    } else if (op == "FIN") {
      res << "(finite_subsets " + translate(fce, enums).at(0) + ")";
    } else if (op == "FIN1") {
      res << "(non_empty_finite_subsets " + translate(fce, enums).at(0) + ")";
    } else if (op == "union") {
      res << "(generalized_union " + translate(fce, enums).at(0) + ")";
    } else if (op == "inter") {
      res << "(generalized_inter " + translate(fce, enums).at(0) + ")";
    } else if (op == "seq") {
      res << "(seq " + translate(fce, enums).at(0) + ")";
    } else if (op == "seq1") {
      res << "(seq1 " + translate(fce, enums).at(0) + ")";
    } else if (op == "iseq") {
      ...
      res << "(size " + translate(fce, enums).at(0) + ")";
    } else if (op == "perm") {
      res << "(perm " + translate(fce, enums).at(0) + ")";
    } else if (op == "first") {
      res << "(first " + translate(fce, enums).at(0) + ")";
    } else if (op == "last") {
      res << "(last " + translate(fce, enums).at(0) + ")";
    } else if (op == "id") {
      res << "(id " + translate(fce, enums).at(0) + ")";
    } else if (op == "closure") {
      res << "(closure " + translate(fce, enums).at(0) + ")";
    } else if (op == "closure1") {
      res << "(closure1 " + translate(fce, enums).at(0) + ")";
    } else if (op == "tail") {
      res << "(tail " + translate(fce, enums).at(0) + ")";
    } else if (op == "front") {
      res << "(front " + translate(fce, enums).at(0) + ")";
    } else if (op == "rev") {
      res << "(rev " + translate(fce, enums).at(0) + ")";
    } else if (op == "struct") {
      throw WhyTranslateException(tag + " " + op);
    } else if (op == "record") {
      throw WhyTranslateException(tag + " " + op);
    } else if (op == "conc") {
      res << "(conc " + translate(fce, enums).at(0) + ")";
    } else if (op == "succ") {
      res << "( " + translate(fce, enums).at(0) + " + 1)";
    } else if (op == "pred") {
      res << "( " + translate(fce, enums).at(0) + " - 1)";
    } else if (op == "rel") {
      res << "(to_relation " + translate(fce, enums).at(0) + ")";
    } else if (op == "fnc") {
      res << "(to_function " + translate(fce, enums).at(0) + ")";
    } else
      throw WhyTranslateException(tag + " " + op);
    return res;
  ...
  if (tag == "Quantified_Pred") {
    if (formula.attribute("type") == "#") {
      res << "(exists " +
                 translateVariables(formula.firstChildElement("Variables"),
                                    enums) +
  ...
  if (tag == "Left" || tag == "Right" || tag == "Body" || tag == "Pred" ||
      tag == "Goal")
  ...
  if (tag == "STRING_Literal") {
    ...
    if (val.isEmpty()) {
      ...
      for (int i = 0; i < val.length(); i++) {
        ...
        l += "(singleton(" + QString::number(i) + "," +
             QString::number(val.at(i).toLatin1()) + "))";
      }
  ...
  if (tag == "Quantified_Exp") {
    ...
    QVector<QString> qTypes;
    QList<whyLocalHyp*> qLocalHyps;
    collectVarAndTypes(&qVariables, &qTypes, formula.firstChildElement("Pred"),
                       enums, qLocalHyps);
    collectVarAndTypes(&qVariables, &qTypes, formula.firstChildElement("Body"),
                       enums, qLocalHyps);
    ...
    VariablesDecl(qVariables, qTypes, formula.firstChildElement("Variables"),
                  &qVariablesQ, &qTypesQ, &qVariablesG, &qTypesG, enums);
    ...
    if (op == "%") {
      ...
      l += ": set((";
      for (int i = 0; i < qVariablesQ.size() - 1; i++) l += "(";
      ...
      l += "),";
      l += translateTypeInfo(
          TypingContext::getType(
              formula.firstChildElement("Body").firstChildElement()),
          enums);
      ...
      for (int i = 0; i < qVariablesG.size(); i++) l += qVariablesG.at(i) + " ";
      l += ") (";
      for (int i = 0; i < qVariablesQ.size() - 1; i++) l += "(";
      for (int i = 0; i < qVariablesQ.size(); i++) {
        if (i > 0) l += "),";
      ...
      nbSigma++;
      return translateSigmaPi(formula, enums, qVariablesG, qTypesG, qVariablesQ,
                              qTypesQ, nbSigma, "sigma", "0", "+");
    } else if (op == "iPI") {
      nbPi++;
      return translateSigmaPi(formula, enums, qVariablesG, qTypesG, qVariablesQ,
                              qTypesQ, nbPi, "pi", "1", "*");
      ...
      l += "xx : " +
           translateTypeInfo(
               TypingContext::getType(formula).firstChildElement(), enums);
  ...
  if (tag == "Boolean_Exp") {
    res << "(if " + translatePred(fce, enums) + "then True else False)";
    return res;
  }

  if (tag == "EmptySet" || tag == "EmptySeq") {
    res << "(empty : " +
               translateTypeInfo(TypingContext::getType(formula), enums) + ")";
    return res;
  }
  ...
  if (tag == "Set") {
    ...
    if (enumE.isNull()) {
      ...
      res << "(mem " + setName + " (finite_subsets integer)) /\\ not(" +
                 setName + "== (empty : set int))";
    } else
      res << "true";
    return res;
  }
...
QStringList whyPredicate::translateSigmaPi(QDomElement formula,
                                           const QMap<QString, QString>& enums,
                                           QVector<QString> qVariablesG,
                                           QVector<QString> qTypesG,
                                           QVector<QString> qVariablesQ,
                                           QVector<QString> qTypesQ, int nbOp,
  ...
  qs.appendChild(
      formula.firstChildElement("Variables").cloneNode().toElement());
  ...
  for (int i = 0; i < qVariablesG.size(); i++) l1 += qVariablesG.at(i) + " ";
  l1 += "(empty : set(";

  for (int i = 0; i < qVariablesQ.size() - 1; i++) l1 += "(";
  for (int i = 0; i < qVariablesQ.size(); i++) {
    if (i > 0) l1 += "),";
    l1 += qTypesQ.at(i);
  }
  l1 += ")) = " + a1 + " \n\n";
  QString l2 =
      "axiom anon_" + op + "_" + QString::number(nbOp) + "_def_ind : forall ";
  first = true;
  for (int i = 0; i < qVariablesG.size(); i++) {
    if (first)
      first = false;
  ...
  for (int i = 0; i < qVariablesQ.size() - 1; i++) l2 += "(";
  for (int i = 0; i < qVariablesQ.size(); i++) {
    if (i > 0) l2 += "),";
    l2 += qTypesQ.at(i);
  }
  ...
  for (int i = 0; i < qVariablesG.size(); i++) l2 += qVariablesG.at(i) + " ";
  l2 += "p_" + op + "_" + QString::number(nbOp);
  l2 += " = anon_" + op + "_" + QString::number(nbOp) + " ";
  for (int i = 0; i < qVariablesG.size(); i++) l2 += qVariablesG.at(i) + " ";
  l2 += "(diff ";
...
QString whyPredicate::translateExtensionSet(
    QDomElement formula, const QMap<QString, QString>& enums) {

  QDomElement sibling = formula.nextSiblingElement();
  if (sibling.isNull())
    return "(singleton " + translate(formula, enums).at(0) + ")";
  else
    return "(union (singleton " + translate(formula, enums).at(0) + ") " +
           translateExtensionSet(sibling, enums) + ")";
}

QString whyPredicate::translateExtensionSeq(QDomElement formula,
                                            const QMap<QString, QString>& enums,
                                            int index) {

  QDomElement sibling = formula.nextSiblingElement();
  if (sibling.isNull())
    return "(singleton (" + QString::number(index) + ", " +
           translate(formula, enums).at(0) + "))";
  else
    return "(union (singleton (" + QString::number(index) + ", " +
           translate(formula, enums).at(0) + ")) " +
           translateExtensionSeq(sibling, enums, index + 1) + ")";
}

QString whyPredicate::VariablesDecl(QVector<QString> variables,
                                    QVector<QString> types, QDomElement formula,
                                    QVector<QString>* variablesQ,
                                    QVector<QString>* typesQ,
                                    QVector<QString>* variablesG,
                                    QVector<QString>* typesG,
                                    QMap<QString, QString> enums) {
  QString res;
...
QString whyPredicate::translateVariables(QDomElement formula,
                                         const QMap<QString, QString>& enums) {
  QString res;
  bool first = true;
  QDomElement fce = formula.firstChildElement("Id");
...
whyLocalHyp::whyLocalHyp(QDomElement d, int _goal, QString _num)
    : whyPredicate(d) {
...
void whyLocalHyp::declareWithoutDuplicate(QList<whyLocalHyp*> lhs,
                                          QTextStream& out) {
  for (int i = 0; i < lhs.size(); i++) {
    if (lhs.at(i)->predicate == predicate &&
      ...
      for (int j = 0; j < variables.size(); j++) {
        dup = (dup && lhs.at(i)->variables.at(j) == variables.at(j) &&
...
QString whyPredicate::declareCall() {
  QStringList args;
  foreach (QString variable, variables) args << variable;
  return "(" + predName + " " + args.join(" ") + ")";
}

whySimpleGoal::whySimpleGoal(QDomElement d, int goal, QList<whyLocalHyp*> wlh,
                             QVector<whyDefine*> wds, whyLocalHyp* hyp)
    : whyPredicate(d, wlh) {
...
void whyLemmaGoal::declare(QTextStream& out) {
  ...
  structs.clear();
  out << "\n";
  for (int i = 0; i < functions.size(); i++) out << functions.at(i) << "\n";
  // pas compatible Qt 5.2: out <<
  // QStringList::fromVector(functions).join("\n");
  out << "\ngoal " << predName << " :\n";
  QStringList tmp;
  if (!variables.isEmpty()) {
    out << "   forall ";
...
static bool isEnumeratedSetElement(const QDomElement& e) {
  return !e.isNull() && e.tagName() == "Set" &&
         !e.firstChildElement("Enumerated_Values").isNull();
}
...
static bool eqType(const QDomElement& e1, const QDomElement& e2) {
  ...
  if (e1.tagName() == "Struct") {
    ...
    while (!c1.isNull() && !c2.isNull()) {
      if (c1.attribute("label") != c2.attribute("label")) return false;
      if (!eqType(child1(c1), child1(c2))) return false;
      c1 = c1.nextSiblingElement("Record_Item");
      c2 = c2.nextSiblingElement("Record_Item");
    }
    return c1.isNull() && c2.isNull();
  }
  if (e1.tagName() == "Generic_Type") return true;

  throw WhyTranslateException("Unknown type element " + e1.tagName());
}

QString whyApply(const QString& fun, const QVector<QString>& args) {
  QString res;
  res = "(" + fun;
  for (int i = 0; i < args.size(); ++i) res += (i > 0 ? " " : "") + args.at(i);
  res += ")";
  return res;
}
